...
---
Link zur Challenge: [link](https://www.kaggle.com/c/m5-forecasting-accuracy)
...
##### Definierung von Helper Functions
...
##### Erstellung des simplen Datensets für naive Ansätze
...
mit zusätzlichen Features, wie Lags
...
##### Looping Louie
The magic loop, forecasting for each product  
1. Naive  
2. SNaive  
3. ARIMA  
4. Random Forest  
...
for (i in 1:iterations){
  ...
  #predict in loop -> Better description needed @moritz
  x = 1
  for (x in 1:28) {
...
Bennenung der Results Matrix & Berechnung des RMSE
...
zur besseren Vergleichbarkeit werden Produkte ausgeschlossen, bei denen ARIMA Fehler aufwies.
...
#### Weighted Results
```{r weighted Results , eval = TRUE, echo = TRUE, error = FALSE, results = "hold"}
...
#### Unweighted Results
```{r ###, eval = TRUE, echo = TRUE, error = FALSE, results = "hold"}
