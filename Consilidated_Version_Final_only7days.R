...
for (i in 1:iterations){
  ...
  predicted = naive(train_product_ts, h = 7)$mean
  ...
  performance_collector <- matrix(ncol=1, nrow=7)
  
  #calculate rmse 
  actual = new_test[1:7,1]
  ...
  #### SNaive Forecast 
  #make forecast and assess RMSE
  snaive_predicted = snaive(train_product_ts, h = 7)$mean
  ...
  try({
    ...
    autoarima_predicted = forecast(autoarima, train_product_ts, h = 7)$mean
  ...
  predictions_matrix_rf = matrix(nrow = 7, ncol = 1)
  ...
  #calculate RMSE
  actual = as.matrix(subset_test_label[1:7,])
  ...
  #initialize predictions matrix to store individual predictions
  predictions_matrix_xgb = matrix(nrow = 7, ncol = 1)
...
mean(results_matrix[1:iterations,3], na.rm = TRUE)
...
Autoarima_result = sum(results_df$Autoarima_weighted, na.rm = TRUE)
